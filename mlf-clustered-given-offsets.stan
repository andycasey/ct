...
data {
    ...
    // In practice we would infer this, but here we are testing.
    vector[D] offset[N]; // the offset due to the clustering  in factor scores
}
...
model {
    ...
    for (n in 1:N) {
        y[n] ~ multi_normal_cholesky(mu - offset[n], L_Sigma);
    }
}